...
func TestGetConflicts(t *testing.T) {
	...
	for _, test := range tests {
		...
		if len(actual) != len(test.expected) {
			t.Fatalf("board.GetConflicts(%d,%d, %d) failed!",
				test.pos.X, test.pos.Y, test.value)
		}
		for i := 0; i < len(actual); i++ {
			if test.expected[i].X != actual[i].X ||
				test.expected[i].Y != actual[i].Y {
				t.Errorf("board.GetConflicts(%d,%d, %d) failed!",
					test.pos.X, test.pos.Y, test.value)
			}
		}
	}
...
func TestGetPositions(t *testing.T) {
	...
	for _, test := range tests {
		...
		if len(actual) != len(test.expected) {
			t.Fatalf("board.GetPositions(%d) failed!", test.value)
		}
		for i := 0; i < len(actual); i++ {
			if test.expected[i].X != actual[i].X ||
				test.expected[i].Y != actual[i].Y {
				t.Errorf("board.GetPositions(%d) failed!", test.value)
			}
		}
	}
}
